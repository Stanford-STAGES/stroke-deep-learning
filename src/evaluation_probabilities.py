...
for fold in fold_folders:
    try:
        ...
        with open(path + fold + '/eval/' + 'probabilities.pkl', 'rb') as f:
            train_group, train_probs, val_group, val_probs, test_group, test_probs, matched_probs, matched_group = pickle.load(f, encoding='latin1')
        ...
        for k, v in train_group.items():
            if v:
                ...
                train_prob_exp.append(train_probs[k])
            else:
                train_label_con.append(v)
                train_prob_con.append(train_probs[k])
        ...
        max_length = np.max([len(e) for e in train_prob_exp + train_prob_con + val_prob + matched_prob_train + matched_prob_test])
        hidden_size = 1
        batch_size_train = 8
        ...
        #x_train = np.zeros(shape=(20, max_length, 1))
        #x_train_exp = [e for e in train_label if e == 1]

        #y_train = np.zeros(shape=(20, 2))
        #for i, e in enumerate(test_prob):
        #    n = len(e)
        #    x_val[i, 0:n, 0] = e
        ...
        best_t = .5 #thress_val[best_t_idx]
